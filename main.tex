...
\begin{figure*}[h]
    ...
    \caption{Halo Mass Function Comparison Across Cosmological Models and Redshifts. The main-text comparison uses FOF halo masses and the Reed07 FOF fitting function. Each panel shows $dn/d\ln M_{\rm FOF}$ as a function of halo mass at a representative redshift. The simulation data are shown as symbols with Poisson error bars, and the matching Reed07 predictions are shown as lines using the same model colors. {\color{blue}Here BT\_soft denotes the model with $k_p=1\,h\,{\rm Mpc}^{-1}$ and $m_s=1.5$, while BT\_deep denotes the model with $k_p=10\,h\,{\rm Mpc}^{-1}$ and $m_s=1.5$.} All simulations use a $25\,h^{-1}{\rm Mpc}$ box with $1024^3$ particles. The corresponding $M_{200c}$ comparison with Bocquet16 is moved to Appendix~\ref{app:m200c_hmf}. {\color{red} In general, we should be more clear about the kp and ms in the plot. What is the kp and ms here? the legend is wrong?} {\color{red} we should focus more on high redshift. We can delete z=0.42, 1.51, 2.67 and add z=10, 15, 20}}
...
The computation of $z_{1/2}$ follows a robust methodology: for each halo, we first determine its mass at $z=0$ ($M_{z=0}$). We then trace its mass evolution backward in time through 21 discrete redshift outputs (from $z=0$ to $z\approx8.5$) along {\color{blue}the main progenitor branch of the HBT+ merger tree}, linearly interpolating between adjacent snapshots to identify the precise redshift when the halo's mass first drops below $M_{z=0}/2$. Halos that do not reach half of their final mass within the redshift range are excluded from the analysis.
