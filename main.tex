...
We evolve the initial conditions with the cosmological $N$-body code \textsc{SWIFT}. In the dark-matter-only mode used here, \textsc{SWIFT} combines short-range gravitational forces with a long-range Particle Mesh treatment for periodic boxes. This setup is appropriate for isolating the gravitational response of the dark matter field to the modified initial spectrum, without introducing baryonic physics or galaxy-formation prescriptions.

\subsubsection{Cosmological Parameters}
\label{subsub:cosmo_params}

All simulations adopt the same background cosmology: $\Omega_{\rm m}=0.3153$, $\Omega_{\rm b}=0.0493$, $\Omega_{\Lambda}=0.6847$, $H_0=67.36\,{\rm km}\,{\rm s}^{-1}\,{\rm Mpc}^{-1}$, $n_s=0.9649$, and $\sigma_8=0.8111$, where $\sigma_8$ is the root-mean-square matter fluctuation in spheres of radius $8\,h^{-1}{\rm Mpc}$. The BT models modify the small-scale primordial tilt while keeping these background parameters fixed.
...
\hline
\hline
Role & Model & $k_p$ & $m_s$ & $N$ & Box size & $m_{\text{DM}}$ & $\epsilon$ \\
 & & $[h\,\text{Mpc}^{-1}]$ & & & $[h^{-1}\,\text{Mpc}]$ & $[h^{-1}M_\odot]$ & $[h^{-1}\,\text{Mpc}]$ \\
\hline
\multicolumn{8}{l}{\textbf{Main comparison runs}} \\
\hline
PL & pl\_25\_1024   & -- & -- & $1024^3$ & 25 & $8.1\times 10^7$ & 0.000145 \\
BT\_soft & kp\_1\_ms\_1.5\_25\_1024  & 1  & 1.5 & $1024^3$ & 25 & $8.1\times 10^7$ & 0.000145 \\
BT\_deep & kp\_10\_ms\_1.5\_25\_1024 & 10 & 1.5 & $1024^3$ & 25 & $8.1\times 10^7$ & 0.000145 \\
\hline
\multicolumn{8}{l}{\textbf{Additional BT variants}} \\
\hline
Steeper BT & kp\_1\_ms\_2.0\_25\_1024  & 1  & 2.0 & $1024^3$ & 25 & $8.1\times 10^7$ & 0.000145 \\
Steeper BT & kp\_10\_ms\_2.0\_25\_1024 & 10 & 2.0 & $1024^3$ & 25 & $8.1\times 10^7$ & 0.000145 \\
Small-box BT & kp\_1\_ms\_1.5\_5\_1024   & 1  & 1.5 & $1024^3$ & 5 & $1.3\times 10^6$ & 0.000029 \\
Small-box BT & kp\_1\_ms\_1.5\_2.5\_1024 & 1  & 1.5 & $1024^3$ & 2.5 & $1.6\times 10^5$ & 0.0000145 \\
\hline
\multicolumn{8}{l}{\textbf{PL convergence and volume tests}} \\
\hline
Resolution & pl\_25\_512    & -- & -- & $512^3$  & 25 & $6.5\times 10^8$ & 0.00029 \\
Resolution & pl\_25\_256    & -- & -- & $256^3$  & 25 & $5.2\times 10^9$ & 0.00058 \\
Volume & pl\_50\_512    & -- & -- & $512^3$  & 50 & $5.2\times 10^9$ & 0.00058 \\
Volume & pl\_256\_1024  & -- & -- & $1024^3$ & 256 & $1.4\times 10^{10}$ & 0.00148 \\
Volume & pl\_256\_512   & -- & -- & $512^3$  & 256 & $1.1\times 10^{11}$ & 0.00296 \\
\hline
\end{tabular}
\caption{Summary of cosmological $N$-body simulation parameters. The main comparison uses the PL, BT\_soft, and BT\_deep runs with identical box size, particle number, particle mass, and force softening. Additional BT variants explore stronger tilts or smaller boxes, while the PL resolution and volume tests provide numerical reference cases. Here $k_p$ is the wavenumber where the primordial spectrum changes slope, $m_s$ is the small-scale spectral index, $N$ is the number of dark matter particles, $m_{\text{DM}}$ is the particle mass, and $\epsilon$ is the gravitational softening length.}
...
Halos and merger trees are identified with \textsc{HBT+}, a particle-tracking halo finder designed to follow bound structures and their progenitors across snapshots. We require a minimum of 20 particles for an object to be retained as a halo candidate, and define the halo center as the position of the most bound particle. Based on the \textsc{HBT+} catalogs, we then use \textsc{SOAP} (Spherical Overdensity and Aperture Processor) to compute spherical-overdensity properties such as $M_{200c}$, $R_{200c}$, and the structural quantities used below. This separation between halo identification and property measurement retains the tracking information from \textsc{HBT+} while adopting a consistent spherical-overdensity definition for the profile and concentration analyses.

\subsection{Mass Definitions and Analysis Samples}
Different statistics use different halo mass definitions. For the halo mass function, we use Friends-of-Friends (FOF) halo masses and compare them to analytic predictions evaluated with the same FOF mass definition. For internal-structure diagnostics, including stacked density profiles and the concentration--mass relation, we use the spherical-overdensity quantities measured by \textsc{SOAP}, primarily $M_{200c}$ and $R_{200c}$. We therefore avoid directly mixing FOF and $M_{200c}$ masses when interpreting a single statistic.

The 20-particle threshold is a catalog-construction threshold, not a precision threshold. In the fiducial $25\,h^{-1}{\rm Mpc}$, $1024^3$ runs, it corresponds to a formal mass of approximately $1.6\times10^9\,h^{-1}M_\odot$. More conservative thresholds of 100 and 300 particles correspond to approximately $8.1\times10^9\,h^{-1}M_\odot$ and $2.4\times10^{10}\,h^{-1}M_\odot$, respectively. The lowest-mass bins should therefore be treated as resolution-sensitive unless they are explicitly supported by the higher-resolution small-box runs.


\subsection{Simulation Suite and Terminology\label{subsec:Terminology}}
The simulation suite is summarized in Table~\ref{tab:simulation_parameters}. The main results compare three runs with the same $25\,h^{-1}{\rm Mpc}$ box, $1024^3$ particles, particle mass, and softening length: PL, BT\_soft, and BT\_deep. PL denotes the standard power-law primordial power spectrum. BT\_soft denotes the blue-tilted model with $k_p=1\,h\,{\rm Mpc}^{-1}$ and $m_s=1.5$, while BT\_deep denotes the blue-tilted model with $k_p=10\,h\,{\rm Mpc}^{-1}$ and $m_s=1.5$. The additional $m_s=2.0$ and small-box BT runs are kept separate from the main comparison and are used only when explicitly stated.
...
For halo abundance statistics, the main low-mass limitation is particle discreteness. Although we retain halo candidates above 20 particles in the catalog construction, the scientific interpretation of the halo mass function is restricted to masses that are sufficiently above this formal detection threshold. Quantities such as the FOF mass and derived abundance become increasingly sensitive to discreteness and halo-finding noise near the low-particle limit. Similarly, the high-mass end is limited by the finite simulation volume, which reduces the number of rare objects and amplifies sample variance.

For internal halo structure, convergence is more restrictive in radius than in total mass. Density profiles and concentrations are sensitive to force softening, particle number, and centering uncertainties in the inner halo. The structural trends discussed below should therefore be interpreted outside the innermost numerically unresolved region and for halo masses that are sufficiently well sampled by particles.

For the nonlinear matter power spectrum, the usable dynamic range is bounded on large scales by the simulation box size and on small scales by force resolution, particle sampling, and grid-assignment effects in the power-spectrum estimator. The highest-$k$ bins should therefore be interpreted with caution unless they are shown to be stable against changes in resolution and estimator settings. Throughout the analysis, we distinguish qualitative trends that appear consistently across diagnostics from precision claims that require stricter convergence cuts.
